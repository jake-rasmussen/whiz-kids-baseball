  ...
  return (
    <>
      <div className="embla" ref={emblaRef}>
        <div className="embla__container grid auto-cols-[100%] grid-flow-col">
          {images.map((image, index) => (
            <div className="embla__slide flex justify-center" key={index}>
              <div className="absolute h-[50rem] lg:h-adjusted w-full bg-gradient-to-b from-red to-white opacity-40" />
              <Image
                priority
                className="h-[50rem] lg:h-adjusted object-cover"
                src={image}
                alt="Whiz Kids Cover"
              />
            </div>
        ...
        <button
          className="embla__prev z-100 absolute left-0 top-[35%] lg:top-[50%] hidden transition duration-300 ease-in-out hover:scale-[200%] md:block"
          onClick={scrollPrev}
        >
          <IconChevronLeft className="h-20 w-20 text-white" />
        </button>
        <button
          className="embla__next z-100 absolute right-0 top-[35%] lg:top-[50%] hidden transition duration-300 ease-in-out hover:scale-[200%] md:block"
          onClick={scrollNext}
        >
